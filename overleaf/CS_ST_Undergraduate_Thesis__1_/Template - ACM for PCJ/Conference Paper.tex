...
%% The abstract
\begin{abstract}
This paper presents the design, implementation, and evaluation of the Multi-Agent System for Flood Route Optimization (MAS-FRO), a comprehensive flood-aware evacuation routing platform for Marikina City, Philippines. Current navigation systems optimize for traffic congestion but fail to account for real-time hydrological hazards, potentially directing evacuees into dangerous flood zones. MAS-FRO addresses this critical gap through three key technical contributions: (1) a novel \textit{Virtual Meters} heuristic that converts normalized risk scores into distance-equivalent units, solving the heuristic domination problem in traditional risk-aware A* implementations; (2) a multi-source data fusion engine integrating official government sensors (PAGASA), crowdsourced social media reports processed via machine learning-based NLP (TF-IDF with sklearn classifiers and spaCy NER), and GeoTIFF-based flood simulations with temporal decay modeling; and (3) a client-side GeoTIFF processing pipeline enabling real-time flood visualization with boundary-aware rendering. The system employs five specialized agents following a Hybrid Multi-Agent System architecture, communicating via FIPA-ACL compliant messaging. Experimental results demonstrate route computation times of 50--500ms on a road network comprising of 16,877 nodes and 35,932 edges, with the Risk-Aware A* algorithm reducing average path risk by 47\% compared to shortest-path baselines while incurring only 23\% additional distance overhead. The system successfully integrates 72 GeoTIFF flood hazard maps representing multiple return periods and achieves real-time WebSocket update latencies of 5--20ms.
\end{abstract}
...
This paper presented MAS-FRO, a multi-agent system for flood-aware evacuation routing in Marikina City. The system successfully integrates heterogeneous data sources: (1) official government sensors, (2) crowdsourced social media reports, and (3) GeoTIFF flood simulations---through a novel data fusion framework with temporal decay modeling. The Virtual Meters heuristic enables intuitive parameterization of safety-efficiency trade-offs in the Risk-Aware A* algorithm. Experimental results demonstrate computation times of 50--500ms, risk reductions of 47\% compared to shortest-path baselines, and real-time update latencies under 20ms.

Looking ahead, future work will focus on addressing current limitations and expanding the system's capabilities. Initially, this involves incorporating real-time traffic data from sources like the Google Maps API or simulation to accurately account for congestion and delays during evacuations. We also plan to enhance the RoutingAgent by introducing multi-vehicle coordination logic to handle simultaneous requests, which should mitigate potential conflicts or bottlenecks. From a data perspective, a critical step is moving toward predictive flood modeling, replacing the reliance on historical GeoTIFF scenarios with more dynamic machine learning-based flood forecasts. To facilitate real-world impact and validation, we intend to develop a mobile application using a React Native interface suitable for field testing with actual evacuees. Finally, we aim for extended coverage, expanding the system's operational area beyond Marikina to serve other flood-prone metropolitan areas in Metro Manila.
